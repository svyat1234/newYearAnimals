...
const UILayer = ({ answeredCount = 0 }) => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/site-content/`)
      ...
      .catch(error => console.error("Failed to fetch site content:", error));
  }, []);
  ...
  return (
    <div className="ui-layer">
      ...
      <section className="promo container">
        <h1 className="promo__title" dangerouslySetInnerHTML={{ __html: content ? content.promoTitle : '' }}></h1>
      ...
      <footer className="footer container">
        <h2 className="footer__title" dangerouslySetInnerHTML={{ __html: content ? content.footerTitle : '' }}></h2>
        <p className="footer__text" dangerouslySetInnerHTML={{ __html: content ? content.footerDescription : '' }}></p>

        <div className="footer__content">
          <div className="footer__copyright-nav">
            <span className="footer__copyright-text">© {new Date().getFullYear()}</span>
